...
class DummyDataProvider : DataProvider() {
    ...
    private val movieListType: Type = object : TypeToken<List<Movie>>() {}.type
    private val gson = Gson()

    init {
        val movie: Movie = gson.fromJson(queryResult, movieType)
        data[movie.id] = movie
    }
    ...
    override fun findMovies(query: String): List<Movie> {
        return gson.fromJson(searchResult, movieListType)
    }

    companion object {
        private const val queryResult =
        ...
        private const val searchResult =
